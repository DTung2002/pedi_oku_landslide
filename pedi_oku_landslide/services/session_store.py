...
class AnalysisContext:
    ...
    def path_in(self, *parts: str) -> str:
        p = os.path.join(self.in_dir, *parts)
        os.makedirs(os.path.dirname(p), exist_ok=True)
        return p

    def path_ui1(self, *parts: str) -> str:
        p = os.path.join(self.out_ui1, *parts)
        os.makedirs(os.path.dirname(p), exist_ok=True)
        return p

    def path_ui2(self, *parts: str) -> str:
        p = os.path.join(self.out_ui2, *parts)
        os.makedirs(os.path.dirname(p), exist_ok=True)
        return p

    def path_ui3(self, *parts: str) -> str:
        p = os.path.join(self.out_ui3, *parts)
        os.makedirs(os.path.dirname(p), exist_ok=True)
        return p
...
def is_valid_run_dir(run_dir: str) -> bool:
    ...
    has_any_ui = any(os.path.isdir(os.path.join(run_dir, d)) for d in ("ui1", "ui2", "ui3"))
...
class PathManager:
    """Compatibility wrapper for older modules still expecting PathManager."""
